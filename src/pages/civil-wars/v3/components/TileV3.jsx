...
function TileV3({
  ...
  // Single status dot colour: green = all met, amber = partial, red = none met
  let dotClass = null;
  if (hasDemand) {
    const unmetCount = SERVICES.filter((s) => tileStat.served[s] < tileStat.demand[s]).length;
    if (unmetCount === 0) dotClass = "cw3-tile-dot--ok";
    else if (unmetCount < SERVICES.length) dotClass = "cw3-tile-dot--partial";
    else dotClass = "cw3-tile-dot--bad";
  }
  ...
  return (
    ...
    >
      ...
      {/* Single status dot — replaces always-on 9-icon strip */}
      {dotClass && <div className={`cw3-tile-dot ${dotClass}`} />}
    </div>
  );
}
